...
pub fn parse_css_to_tree(content: &str, _file_path: &str) -> Result<Vec<Rc<TreeNode>>, String> {
    ...
    let tree = parser.parse(content, None).ok_or_else(|| "Failed to parse CSS".to_string())?;
...
pub fn parse_scss_to_tree(content: &str, _file_path: &str) -> Result<Vec<Rc<TreeNode>>, String> {
    ...
    let tree = parser.parse(content, None).ok_or_else(|| "Failed to parse SCSS".to_string())?;
...
fn rule_to_tree(node: &Node, source: &str) -> Option<Rc<TreeNode>> {
    ...
    let mut tree_node = TreeNode::new("rule".to_string(), selector_text.to_string(), 0);
...
fn at_rule_to_tree(node: &Node, source: &str) -> Option<Rc<TreeNode>> {
    ...
    let mut tree_node = TreeNode::new("at-rule".to_string(), at_keyword.to_string(), 0);
    tree_node.children = children;
    Some(Rc::new(tree_node))
}
...
fn mixin_to_tree(node: &Node, source: &str) -> Option<Rc<TreeNode>> {
    ...
    let mut tree_node = TreeNode::new("mixin".to_string(), format!("@mixin {name}"), 0);
    tree_node.children = children;
    Some(Rc::new(tree_node))
}
...
fn declaration_to_tree(node: &Node, source: &str) -> Option<Rc<TreeNode>> {
    let property =
        node.child_by_field_name("property").and_then(|n| n.utf8_text(source.as_bytes()).ok())?;
