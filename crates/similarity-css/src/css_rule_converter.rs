    ...
    for line_idx in start_idx..end_idx {
        if let Some(line) = lines.get(line_idx) {
            ...
            if let Some(colon_pos) = line.find(':') {
                ...
                if !property.contains('{') && !property.contains('}') && !value.contains('{') && !value.is_empty() {
...
pub fn parse_css_to_rules(content: &str, file_path: &str) -> Result<Vec<CssRule>, Box<dyn std::error::Error + Send + Sync>> {
    let mut parser = CssParser::new();
    let functions = parser.extract_functions(content, file_path)?;
    
    Ok(functions.iter()
        .map(|func| convert_to_css_rule(func, content))
        .collect())
}
