...
fn are_fingerprints_similar(fp1: &str, fp2: &str) -> bool {
    let parts1: HashMap<&str, &str> = fp1.split(',')
        .filter_map(|p| {
    ...
    let parts2: HashMap<&str, &str> = fp2.split(',')
        .filter_map(|p| {
            let mut iter = p.split(':');
            Some((iter.next()?, iter.next()?))
        })
...
fn compare_types(
    ...
    // Parse to AST
    let tree1 = parser.parse(&type1_code, &type1.file_path)
        .map_err(|e| anyhow::anyhow!("Failed to parse type1: {}", e))?;
    let tree2 = parser.parse(&type2_code, &type2.file_path)
...
pub fn check_types(
    ...
    let extracted_types: Vec<ExtractedType> = files
        ...
        .flat_map(|file| {
            ...
            Some(types.into_iter().map(move |type_def| ExtractedType {
                type_def,
